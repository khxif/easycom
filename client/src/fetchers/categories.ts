...
export const updateCategories = async (options: { id: string; category: CategorySchemaType }) => {
  const data = await apiClient.put(`/categories/${options?.id}`, options?.category);
  return data;
};
